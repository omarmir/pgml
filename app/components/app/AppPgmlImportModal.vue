...
const activeInputSummary = computed(() => {
  ...
  if (hasPastedText.value) {
    return 'Import will use the pasted PGML text.'
  }

  return 'Choose a file or paste PGML text to begin the import.'
})
...
<template>
  <StudioModalFrame
    :open="open"
    title="Import PGML document"
    description="Paste PGML or upload a PGML file. VersionSet imports append their locked versions, comparisons, metadata, and current workspace to this document."
    surface-id="pgml-import"
    ...
    <div class="grid gap-4">
      <div :class="importRulesClass">
        ...
        <p :class="studioBodyCopyClass">
          Choose exactly one input method. This appends the imported PGML history and makes the imported workspace the current draft.
        </p>
