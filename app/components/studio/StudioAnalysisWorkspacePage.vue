...
const workspaceMode: StudioWorkspacePage = 'analysis'
...
const {
  editorRef,
  focusEditorDiagnostic: _focusEditorDiagnostic,
...
const {
  ...
  compareTargetSource,
  canCheckpoint,
  canDeleteDiagramView: _canDeleteDiagramView,
  ...
  diagramViewItems,
  diagramViewSettings: _diagramViewSettings,
...
const { isCompactStudioLayout } = useStudioEditorLayout()
...
const _versionedEditorModeItems = [
  {
...
const _documentEditorScopeItems = computed<PgmlVersionedDocumentScopeItem[]>(() => {
...
const _editorCommitDebounceMs = computed(() => {
...
const _editorDiagnosticsDelayMs = computed(() => {
  if (!isEditableWorkspaceDraft.value) {
    return 150
  }
...
const _editorActivateCompletionOnTyping = true
...
const _sourceWarningDiagnostics = computed(() => {
  return sourceDiagnostics.value.filter(diagnostic => diagnostic.severity === 'warning')
})
const _hasBlockingSourceErrors = computed(() => sourceErrorDiagnostics.value.length > 0)
...
const _editorReadOnlyLabel = computed(() => {
...
const _editorModeDescription = computed(() => {
  if (versionedEditorMode.value === 'document') {
...
const _mobileCanvasView = computed<StudioMobileCanvasView>(() => {
...
const _assignEditorRef = (value: unknown) => {
  editorRef.value = value as PgmlSourceEditorHandle | null
}
const _assignLayoutShellRef = (_value: unknown) => {
}
...
const _syncSourceWithNodeProperties = (nodeProperties: Record<string, PgmlNodeProperties>) => {
...
const _updateDiagramViewSettings = (settings: {
  snapToGrid?: boolean
...
const _selectActiveDiagramView = (viewId: string) => {
  markBrowserSchemaStatusEligible()
...
const _createActiveDiagramView = () => {
  openCreateDiagramViewDialog()
}

const _renameSelectedDiagramView = () => {
  openRenameDiagramViewDialog()
}

const _deleteSelectedDiagramView = () => {
  markBrowserSchemaStatusEligible()
...
const _updateVersionedEditorMode = (value: string) => {
  versionedEditorMode.value = normalizeVersionedEditorMode(value)
}
const _updateDocumentEditorScope = (value: unknown) => {
...
const actionMenus = computed<StudioHeaderMenu[]>(() => {
  const _exportItems: DropdownMenuItem[] = exportScales.map((scaleOption) => {
    return {
...
const _openTableEditor = (tableId: string) => {
  if (!isWorkspacePreview.value || workspaceHasBlockingSourceErrors.value) {
    return
  }
...
const _openTableCreator = (groupName: string | null) => {
  if (!isWorkspacePreview.value || workspaceHasBlockingSourceErrors.value) {
    return
  }
...
const _openGroupEditor = (groupName: string) => {
  if (!isWorkspacePreview.value || workspaceHasBlockingSourceErrors.value) {
    return
  }
...
const _handleCanvasFocusSource = async (sourceRange: PgmlSourceRange) => {
  await flushPendingEditorChanges()
...
const _handleCanvasReplaceSourceRange = (input: {
  nextText: string
...
const _handleCanvasPanelTabChange = (nextTab: DiagramPanelTab) => {
  mobilePanelTab.value = nextTab
}

const _handleCanvasToolPanelTabChange = (nextTab: DiagramToolPanelTab) => {
  mobileToolPanelTab.value = nextTab
}

const _handleCanvasToolPanelVisibilityChange = (payload: {
  open: boolean
  tab: DiagramToolPanelTab
}) => {
...
const _handleMobileCanvasViewChange = (nextView: StudioMobileCanvasView) => {
