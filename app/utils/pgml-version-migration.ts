...
const noAutomaticSqlStepComment = 'No automatic SQL statements were generated for this step. Review warnings.'
const sqlWarningPrefix = '-- Warning: '
const kyselyWarningPrefix = '  // Warning: '
const sqlStepPrefix = '-- Step '
const kyselyStepPrefix = '  // Step '
...
const buildVersionHistoryKyselyContent = (
  ...
  const stepBlocks = steps
    .map((step) => {
      const warningLines = step.plan.warnings.map(warning => `${kyselyWarningPrefix}${warning}`)
...
\`.execute(db)`
      ...
      return `${kyselyStepPrefix}${step.index + 1}: ${step.label}\n${[...warningLines, body].join('\n')}`
    })
    .join('\n\n')
