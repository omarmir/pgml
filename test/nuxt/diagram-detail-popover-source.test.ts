...
describe('diagram detail popover source', () => {
  ...
  it('adds a structured metadata editor next to the raw popup source editor', () => {
    ...
    expect(shellFile).toContain('const detailPopoverViewportInsetTop = computed(() => {')
    expect(shellFile).toContain('const detailPopoverViewportInsetBottom = computed(() => {')
    expect(shellFile).toContain('data-detail-popover-body="true"')
    expect(shellFile).toContain('grid max-h-full min-h-0 w-full grid-rows-[auto_minmax(0,1fr)_auto]')
    expect(metadataEditorFile).toContain('data-detail-popover-metadata-editor="true"')
    expect(metadataEditorFile).toContain('const unsetSelectValue = \'__pgml_detail_metadata_unset__\'')
    expect(metadataEditorFile).toContain('{ label: \'Auto / unset\', value: unsetSelectValue }')
