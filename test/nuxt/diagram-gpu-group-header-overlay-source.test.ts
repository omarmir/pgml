...
describe('diagram GPU group header overlay source', () => {
  it('renders group headers in a dedicated layer above connections and below tables', () => {
    ...
    expect(sceneFile).toContain('headerGradient.addColorStop(headerFadeOutStop, \'rgba(0, 0, 0, 0)\')')
  ...
  it('invalidates cached group sprites when the group render content changes', () => {
    ...
    expect(sceneFile).toContain('const nextKey = `${key}:${getSelectionStateKey(key)}:${Math.round(resolution * 100)}:${width}x${height}:${hashDiagramRenderSignature(renderSignature)}:${hashDiagramRenderSignature(sceneThemeSignature)}`')
